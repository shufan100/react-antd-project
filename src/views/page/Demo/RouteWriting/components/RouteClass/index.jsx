  ...
  render() {
    ...
    return (
      <div>
        ...
        <Link to={'/abouts'}> 作者 </Link>
        <br />
        <br />

        {/* 编程式路由 */}
        <button onClick={e => history.push('/demo/param/9523/tom')}>Param传参 (push)</button>
        <button onClick={e => history.replace('/demo/param/8877/helen')}>Param传参 (replace)</button>
        <br />
        <br />

        <button onClick={e => history.push(`/demo/search?id=${'9877'}&title22=${'小张'}`)}>Search传参 (push)</button>
        <button onClick={e => history.replace(`/demo/search?id=${'15138'}&title22=${'警察'}`)}>Search传参 (replace)</button>
        <br />
        <br />

        <button onClick={e => history.push({ pathname: '/demo/state', state: { id: '9999', name: '小鸡' } })}>State传参 (push)</button>
        <button onClick={e => history.replace({ pathname: '/demo/state', state: { id: '7572', name: '吉马' } })}>State传参 (replace)</button>
        <br />
        <br />
      </div>
    )
